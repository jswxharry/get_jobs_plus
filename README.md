...
本库是在2026年1月8日fork自原库，因原库交流群满了进不去了，原作者申明说有的变更需要提前和他沟通，因此只能fork个版本，根据本人的想法后续做调整：
1. 完善本地配置启动文档，以及依赖安装说明，更加面向小白玩家，逐步更新进 [How to Start](doc/How_to_start.md)
2. IDE 迁移为 基于 VS Code，因为这个库里还有前端界面，不只是Java后端服务，而且本人现有的AI 编码是用VS Code Roo Code插件 + Kimi Coding，这也是其中一个原因

其他初步使用发现的功能性问题，会后续逐步改良：
- boss直聘网页抓取频率太高，触发机器人检测，两次后账号被暂时暂停，会后续看看如何解决。
- 没有看到一个职位清单，会后续看看如何进一步容易获得
